...
describe("Pedido Use Case", () => {
    ...
    it("executa update preparacao", async () => {
        const pedido: any = await pedidoUseCase.executeUpdatePedidoPreparacao(
            1
        );
    ...
    it("executa get pedidos by fake checkout", async () => {
        const pedido: any = await pedidoUseCase.executeGetPedidoFakeCheckout(
            "Pronto"
        );
    ...
    it("executa add Produtos ao pedido", async () => {
        ...
        const pedido: any = await pedidoUseCase.executeAddProdutosAoPedido([
            produtos,
        ]);
    ...
    it("executa remove produto do pedido", async () => {
        ...
        const response = await pedidoUseCase.executeRemoveProdutoDoPedido(
            idPedido,
            idProduto
        );
    ...
    it("executa update pedido finalizado", async () => {
        ...
        const idPedido = 1; // ID do pedido fictício
        const response = await pedidoUseCase.executeUpdatePedidoFinalizado(
            idPedido
        );
        ...
        expect(mockPedidoGateway.updatePedidoCompleto).toHaveBeenCalledWith(
            pedidoParaAtualizar
        );
    ...
    it("executa update pedido finalizado com valor zero", async () => {
        ...
        await expect(
            pedidoUseCase.executeUpdatePedidoFinalizado(idPedido)
        ).rejects.toThrow(
    ...
    it("executa getProdutoDoPedido com erro", async () => {
        ...
        jest.spyOn(
            mockProdutoDoPedidoGateway,
            "getProdutosDoPedido"
        ).mockImplementation(async (idPedido) => {
        ...
        await expect(
            pedidoUseCase.executeGetProdutoDoPedido(idPedido)
        ).rejects.toThrow(`Erro ao buscar Produtos do Pedido ${idPedido}`);

        // Verifica se a mensagem de erro foi registrada no console
        expect(console.error).toHaveBeenCalledWith(
            new Error("Erro simulado ao buscar produtos do pedido")
        );
    ...
    it("executa update pedido finalizado com erro ao criar pagamento", async () => {
        ...
        jest.spyOn(pedidoUseCase, "calculaValorDoPedido").mockResolvedValue(
            100
        );

        // Mock createPagamento para lançar um erro
        const createPagamentoError = new Error(
            "Erro simulado ao criar pagamento"
        );
