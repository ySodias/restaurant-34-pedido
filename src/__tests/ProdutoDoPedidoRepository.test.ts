...
describe("Produto do pedido", () => {
    ...
    it("create fail", async () => {
        ...
        prismaMock.produtosDoPedido.createMany.mockRejectedValue(new Error('Error'))
        await expect(
    ...
    it("get fail", async () => {
        ...
        prismaMock.produtosDoPedido.findMany.mockRejectedValue(new Error('Error'))
        await expect(
            produtosDoPedidoRepository.get(2)
        ).rejects.toThrow()
    })
})
