...
describe("Produto Do Pedido", () => {
    ...
    beforeAll(async () => {
        produtoDoPedidoGateway = new ProdutoDoPedidoGateway(
            mockProdutosDoPedidoRepository
        );
    });
    ...
    it("createProdutosDoPedido", async () => {
        ...
        const produtoDoPedidoCriado: any =
            await produtoDoPedidoGateway.createProdutosDoPedido([produto2]);
    ...
    it("deleteProdutosDoPedido", async () => {
        ...
        const produtoDoPedidoDeletado: any =
            await produtoDoPedidoGateway.deleteProdutosDoPedido([produto2]);

        expect(produtoDoPedidoDeletado).toBeDefined();
    });

    it("getProdutosDoPedido", async () => {
        const getProdutosDoPedido: any =
            await produtoDoPedidoGateway.getProdutosDoPedido(1);

        expect(getProdutosDoPedido).toBeDefined();
    });

    it("removeProdutoDoPedido", async () => {
        const removeProdutoDoPedido: any =
            await produtoDoPedidoGateway.removeProdutoDoPedido(1, 1);
